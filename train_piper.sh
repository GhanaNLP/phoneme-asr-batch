...
# metadata_train.csv is pipe-delimited: utt|speaker|text|phoneme_ids
NSPK=$(cut -d'|' -f2 "$DATA/metadata_train.csv" | sort -u | grep -c .)
if [ "$NSPK" -lt 2 ]; then
  echo "refusing to train: found only $NSPK speaker(s) in $DATA/metadata_train.csv" >&2
  exit 1
fi
echo "speakers: $NSPK"
...
exec $PY -m piper.train fit \
  ...
  --data.csv_path "$DATA/metadata_train.csv" \
  --data.audio_dir "$DATA/wav" \
  --data.dataset_type phoneme_ids \
  --data.phonemes_path "$DATA/phonemes.json" \
